...
test("it should navigate to item on CardsGrid click", async (t) => {
  await navigateTo(t, "shows");
  await t
    .click(cardSelector)
    .expect(getPageUrl())
...
test("it should move card selector left and right", async (t) => {
  ...
  await t
    .pressKey("right")
    .expect(
      Selector(".Card").nth(1).withAttribute("class", "Card Card--selected")
        .visible
    )
    .ok()
    .pressKey("left")
    .expect(
      Selector(".Card").nth(0).withAttribute("class", "Card Card--selected")
        .visible
    )
    .ok()
    .pressKey("left")
